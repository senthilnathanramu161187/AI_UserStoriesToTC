...
generateRouter.post('/', async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    // Validate categories if provided by client
    if (request.categories && Array.isArray(request.categories) && request.categories.length === 0) {
      res.status(400).json({ error: 'If provided, categories must contain at least one value' })
      return
    }

    // Build prompts (buildPrompt will include categories if present)
